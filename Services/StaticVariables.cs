namespace McSharesAPI.Services
{
    public class StaticVariables
    {
        public static string customerTypeIndividual = "Individual";
        public static string errorCustomerNotFound = "Customer not found";
        public static string errorIncorrectId = "Customer Id is incorrect";
        public static string errorNotAmendableId = "Customer Id is not amendable";
        public static string errorNumShares = "Number of Shares is not amendable if the customer is a Corporate";
        public static string errorNoCustomersFoundName = "No customers found by this name";
        public static string errorNoCustomersFound = "No customers found";
    }
}